<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        $new_file_name=strtolower($reg_no."-demo");
        $final_file=str_replace(' ','-',$new_file_name);
        if(move_uploaded_file($file_loc,$folder.$final_file)){ 
            $sql="UPDATE student_credentials SET demo='$final_file' WHERE reg_no='$reg_no'";
        ...
        if($count==0){
            ...
            while($count>0){
                    ...
                    if($row["demo"]==NULL){
                        ...
                        echo "<a href='./uploads/demo/".$row["demo"]."' target='_blank'>".$row["demo"]."</a>";
                    }
                    $count=$count-1;
                }
        }
...
<div class="container">
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])?>" name="upload_ppt" method="post" enctype="multipart/form-data">
            <div class="row">
                <div class="col-25">
                    <label for="file">Upload Demo Video</label>
                </div>
                <div class="col-75">
